  ...
  while (endpoint) {
    try {
      ...
      if (data.articles) {
        ...
        console.log(`📄 Fetched ${data.articles.length} articles from this page`);
      }
...
for (const article of articles) {
  try {
    ...
    if (!markdownBody || markdownBody.trim() === '') {
      console.log(`⏭️ Skipping article ${article.id} - no content after HTML conversion`);
      continue;
    }
    ...
    await pgVector.upsert({
      ...
      metadata: chunks.map((chunk) => ({
        text: chunk.text,
        source: article.html_url || `https://${ZENDESK_SUBDOMAIN}.zendesk.com/hc/pt-br/articles/${article.id}`,
        article_id: article.id,
        article_title: title,
      })),
    });

    console.log(`✅ Article ${article.id} ("${title}") processed successfully!`);
